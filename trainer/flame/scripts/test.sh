#!/usr/bin/bash

export HF_ENDPOINT=https://hf-mirror.com
export HF_HOME=/ssdwork/lisiyuan/hf_cache/
export BACKBONE=transformer++_340M
echo $BACKBONE

# ------ jinxin val test ------ #
NNODE=1 NGPU=1 LOG_RANK=0 bash train.sh \
  --job.config_file flame/models/fla.toml \
  --job.dump_folder exp/test \
  --model.config configs/transformer_340M.json \
  --model.tokenizer_path /lisiyuan/jx/.cache/transformer-1.3B-100B \
  --optimizer.name AdamW \
  --optimizer.eps 1e-15 \
  --optimizer.lr 3e-4 \
  --lr_scheduler.warmup_steps 1024 \
  --lr_scheduler.lr_min 0.1 \
  --lr_scheduler.decay_type cosine \
  --training.val_times 2 \
  --training.val_data_dir /lisiyuan/jx/OpenToMe/data/wiki_val \
  --training.batch_size 1 \
  --training.seq_len 10 \
  --training.context_len 1024 \
  --training.varlen \
  --training.gradient_accumulation_steps 2 \
  --training.steps 10 \
  --training.max_norm 1.0 \
  --training.skip_nan_inf \
  --training.dataset /liziqing/fineweb-edu/sample/100BT \
  --training.dataset_name default \
  --training.dataset_split train \
  --training.num_workers 32 \
  --training.prefetch_factor 2 \
  --training.seed 42 \
  --training.compile \
  --checkpoint.interval 15360 \
  --checkpoint.load_step -1 \
  --checkpoint.keep_latest_k 2 \
  --metrics.log_freq 1

# ------ jinxin C4 dataset------ #
# NNODE=1 NGPU=1 LOG_RANK=0 bash train.sh \
#   --job.config_file flame/models/fla.toml \
#   --job.dump_folder exp/test_c4 \
#   --model.config configs/transformer_340M.json \
#   --model.tokenizer_path /lisiyuan/jx/.cache/transformer-1.3B-100B \
#   --optimizer.name AdamW \
#   --optimizer.eps 1e-15 \
#   --optimizer.lr 3e-4 \
#   --lr_scheduler.warmup_steps 1024 \
#   --lr_scheduler.lr_min 0.1 \
#   --lr_scheduler.decay_type cosine \
#   --training.val_times 2 \
#   --training.batch_size 1 \
#   --training.seq_len 10 \
#   --training.context_len 1024 \
#   --training.varlen \
#   --training.gradient_accumulation_steps 2 \
#   --training.steps 10 \
#   --training.max_norm 1.0 \
#   --training.skip_nan_inf \
#   --training.dataset_name default \
#   --training.dataset_split train \
#   --training.dataset json \
#   --training.data_files '/lisiyuan/data/c4/en/c4-train.*.json.gz' \
#   --training.streaming \
#   --training.num_workers 32 \
#   --training.prefetch_factor 2 \
#   --training.seed 42 \
#   --training.compile \
#   --checkpoint.interval 15360 \
#   --checkpoint.load_step -1 \
#   --checkpoint.keep_latest_k 2 \
#   --metrics.log_freq 1