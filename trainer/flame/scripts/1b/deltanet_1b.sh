...
#    1 (BS) * 32768 (SeqLen) * 8 (GPU) * 8 (GA) = 2,097,152 (2M Tokens)
# 2. Warmup 步数 (1B 目标):
...
NNODE=1 NGPU=8 LOG_RANK=0 bash train.sh \
  ...
  --job.dump_folder exp/delta_net_1B_100B/batch1.seqlen32768.grad_acc8.warmup477.update1.steps47684.lr3e-4 \
  --model.config configs/gated_deltanet_1B.json \
  --model.tokenizer_path /lisiyuan/jx/.cache/gla-1.3B-100B \
