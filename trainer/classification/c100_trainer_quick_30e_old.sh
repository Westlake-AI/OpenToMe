#!/bin/bash
# bash c100_trainer_quick_30e_old.sh 2>&1 | tee train_log_$(date +%Y%m%d_%H%M%S).txt
...
OPENTOME_MERGENET_IMPL=old CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --standalone --nproc_per_node 8 "${SCRIPT_DIR}/in1k_trainer.py" \
  --data_dir ${DATA_DIR} \
  --dataset CIFAR100 \
  --train_split train \
  --val_split val \
