...
class Adam_mini(torch.optim.Optimizer):
    def __init__(
        ...
        '''
        # Handle both named_parameters (tuples) and regular parameters
        named_parameters_list = list(named_parameters)
        if named_parameters_list and not isinstance(named_parameters_list[0], tuple):
            named_parameters_list = [(f"param_{i}", p) for i, p in enumerate(named_parameters_list)]
        self.named_parameters = named_parameters_list
        named_parameters = named_parameters_list
        self.dim = dim
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if any(adam_block_name in name for adam_block_name in
                    ...
                    # 1D Q/K params (e.g. bias) cannot be reshaped into heads. Fall back to standard Adam.
                    if grad.ndim < 2:
                        if len(state) == 0:
                            state["m"] = torch.zeros_like(p, memory_format=torch.preserve_format)
                            state["step"] = 0
                    ...
                    # 1D parameters (e.g. bias terms whose name contains mlp/attn keywords)
                    # cannot use per-neuron lr (requires dim=1 indexing). Fall back to standard Adam.
                    if grad.ndim < 2:
                        if len(state) == 0:
                            state["m"] = torch.zeros_like(p, memory_format=torch.preserve_format)
                            state["step"] = 0
