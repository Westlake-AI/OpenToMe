...
# adam_mini_fixed.py
# A fixed/robust Adam-mini style optimizer (no SAC scale) that handles 1D/2D param shapes safely.
# Usage: optimizer = AdamMiniFixed(model.parameters(), lr=1e-3, betas=(0.9,0.999), eps=1e-8, weight_decay=0.0, dim=512, n_heads=8)
...
import torch.nn as nn
import torch.distributed as dist
from torch.distributed._tensor import Replicate  # optional DTensor support

# device fallback (used only in some scalar constructions)
device = 'cuda' if torch.cuda.is_available() else 'cpu'


class Adam_mini(torch.optim.Optimizer):
    """
    Adam-mini style optimizer (fixed).
    - Robust vmean initialization (no multi-indexing on 1D tensors).
    - Head-wise and neuron/row-wise second-moment aggregation for attention / mlp.
    - Block-wise fallback for other params (LayerNorm, biases).
    - Optional gradient alignment.
    - No SAC / scaling logic in this version.
    """
    ...
    def __init__(
        ...
        *,
        ...
        assert (self.dim * self.dim) % self.n_heads == 0, "dim*dim must be divisible by n_heads"
        self.head_numel = (self.dim * self.dim) // self.n_heads
        ...
        # 名称分类
        self.embedding_names = {"embed_tokens", "embedding"}
        ...
        self.norm_names = {"norm", "layernorm"}
        self.adam_block_names = {"bias"}

        # ---- 把 named_parameters 转换为 param_groups ----
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if any(k in name for k in self.adam_block_names):
                    ...
                    p.addcdiv_(state["m"], denom, value=-lr / bc1)

                # ---- (B) q_proj/k_proj: head-wise ----
                elif any(k in name for k in {"q_proj", "k_proj"}):
                    if len(state) == 0:
                        m = torch.zeros_like(p)
                        state["m"] = m.view(-1, self.head_numel)
                        state["head_per_gpu"] = state["m"].size(0)
                        state["vmean"] = torch.zeros(state["head_per_gpu"], 1, device=p.device, dtype=p.dtype)
                        state["step"] = 0

                    gflat = g.contiguous().view(state["head_per_gpu"], -1)
                    tmp = torch.mean(gflat * gflat, dim=1, keepdim=True)
                    state["vmean"].mul_(beta2).add_(tmp, alpha=1 - beta2)
                    state["m"].lerp_(gflat, 1 - beta1)
                    state["step"] += 1
                    ...
                    stepsize = ((1 / bc1) / h).view(state["head_per_gpu"], 1)
                    update = (state["m"] * stepsize).view_as(p)
                    p.add_(-lr * update)

                # ---- (C) embedding/mlp/v_proj/o_proj/head: neuron-wise (only 2D) ----
                elif (
                    (any(k in name for k in self.embedding_names)
                    ...
                    bc2 = 1 - beta2 ** state["step"]
                    h = (state["vmean"].sqrt() / math.sqrt(bc2)).add_(eps)
                    stepsize = ((1 / bc1) / h).view(state["neuron_per_gpu"], 1)
                    update = (state["m"] * stepsize).view_as(p)
                    p.add_(-lr * update)

                # ---- (D) fallback: block-wise ----
                else:
                    if len(state) == 0:
                        state["m"] = torch.zeros_like(p)
                        state["vmean"] = torch.zeros(1, device=p.device, dtype=p.dtype)
                        state["block_numel"] = float(p.numel())
                        state["step"] = 0
