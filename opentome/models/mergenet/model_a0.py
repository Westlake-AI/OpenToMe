...
from opentome.timm.tome import ToMeLocalBlock, tome_apply_patch
from opentome.timm.bias_local_attn import LocalBlock
from opentome.models.mergenet.model import LatentEncoder


class ToMEViTLocalEncoder(nn.Module):
    """
    Local stage: timm ViT blocks + tome_apply_patch（ToMe 在 attn 与 MLP 之间）。
    若 local_attn_window 为整数，则块内为窗口注意力 + ToMe（ToMeLocalBlock），不再对标准 Block 打补丁。
    """
    ...
    def __init__(
        ...
        attn_drop_rate=0.0,
        drop_path_rate=0.0,
        total_merge_local: int = 0,
        tome_window_size=None,
        tome_use_naive_local: bool = False,
        ...
        if local_attn_window is not None:
            ...
            self.vit._tome_info = {
                ...
                "window_size": tome_window_size,
                "use_naive_local": bool(tome_use_naive_local),
            }
            for blk in self.vit.blocks:
                blk._tome_info = self.vit._tome_info
        else:
            tome_apply_patch(
                self.vit,
                trace_source=True,
                prop_attn=True,
                window_size=tome_window_size,
                use_naive_local=tome_use_naive_local,
                r=self.default_r,
            )
            self.vit._tome_info["total_merge"] = total_merge_local
    ...
    def forward(self, x):
        ...
        self.vit._tome_info["r"] = parse_r(
            len(self.vit.blocks), self.vit.r, self.total_merge_local
        )
        ...
        x_out = self.vit.norm(x)
        x_embed = x
        size = self.vit._tome_info["size"]
        info = {
            "source_map": self.vit._tome_info.get("source_map"),
...
class ToMELocalEncoder(nn.Module):
    ...
    结构：先顺序堆叠 n 层 transformer（LocalBlock 窗口注意力，或 use_local_window=False 时 timm 全局 Block），
    全部算完后在「最后一层的输出」上按 parse_r 做若干次链式 post-hoc 合并（不是「第 k 层输出接第 k 次 merge」）。
...
class ToMEHybridModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 local_encoder_type: str = "localblock",
                 use_local_window: bool = True,
                 vit_tome_local_attn_window=None,
                 **kwargs):
        super().__init__()
        ...
        self.latent_use_tome = total_merge_latent > 0
        ...
        self.local_encoder_type = local_encoder_type
        self.use_local_window = use_local_window
        self.vit_tome_local_attn_window = vit_tome_local_attn_window

        self.num_classes = num_classes

        if local_encoder_type == "vit_tome":
            self.local = ToMEViTLocalEncoder(
                self.img_size,
                self.patch_size,
                self.embed_dim,
                self.num_heads,
                ...
                tome_window_size=self.tome_window_size,
                tome_use_naive_local=self.tome_use_naive_local,
                local_attn_window=self.vit_tome_local_attn_window,
            )
        elif local_encoder_type == "localblock":
            self.local = ToMELocalEncoder(
                ...
                use_local_window=self.use_local_window,
            )
        else:
            raise ValueError(
                f"local_encoder_type must be 'localblock' or 'vit_tome', got {local_encoder_type!r}"
            )
    ...
    def _apply_patches(self):
        if (
            ...
            and len(self.latent.vit.blocks) > 0
            and self.latent_use_tome
        ):
...
@register_model
def tomevit_small_cls_ax(**kwargs):
    """Ablate local stage: timm ViT blocks + tome_apply_patch (vs LocalBlock + post-hoc merge)."""
    return CLSToMEHybridModel(arch='small', local_encoder_type='vit_tome', **kwargs)


@register_model
def tomevit_small_cls_a0_global(**kwargs):
    """Post-hoc ToME + 全局 timm Block（无 LocalBlock 窗口注意力）。"""
    merged = dict(kwargs)
    merged['use_local_window'] = False
    return CLSToMEHybridModel(arch='small', **merged)


@register_model
def tomevit_small_cls_ax_local(**kwargs):
    """块内 ToME + 窗口注意力（ToMeLocalBlock）；窗口边长默认与 local_block_window 一致。"""
    merged = dict(kwargs)
    merged['local_encoder_type'] = 'vit_tome'
    if merged.get('vit_tome_local_attn_window', None) is None:
        merged['vit_tome_local_attn_window'] = merged.get('local_block_window', 16)
    return CLSToMEHybridModel(arch='small', **merged)
